...
int export_vbm()
{
	...
	FILE *vbm_out;
	if (LEVEL==0)
		vbm_out = fopen("Kwirk (UA) [optimized].vbm", "wb");
	else
		vbm_out = fopen("Kwirk (UA) [optimized].vbm", "ab");
	if (!vbm_out)
	{
		fclose(solution_in);
	...
	if (LEVEL == 0)
	{
		static BYTE vbm_header[0x100] = {
			0x56,0x42,0x4D,0x1A,0x01,0x00,0x00,0x00,0x62,0xEE,0xB2,0x45,0xC7,0xDD,0x00,0x00,
		...
		strcpy((char*)(vbm_header+0x40), "CyberShadow and Deadcode");
